...
from results.results_unetplusplus_exp_B.unetplusplus import SmallNestedUNet  # Update import to the smaller version
...
# Create the model
model = SmallNestedUNet(num_classes=1, input_channels=3, deep_supervision=False)
...
criterion = nn.BCEWithLogitsLoss()  # Since num_classes=1, use BCEWithLogitsLoss
...
writer = SummaryWriter('runs/unetplusplus_experiment_BB')
...
for epoch in range(num_epochs):
    ...
    for imgs, masks in progress_bar:
        ...
        if model.deep_supervision:
            loss = sum([criterion(output.squeeze(), masks.squeeze()) for output in outputs]) / len(outputs)
        else:
        ...
        if model.deep_supervision:
            for output in outputs:
                iou += iou_score(output.squeeze(), masks.squeeze())
                f1_score += f1score(output.squeeze(), masks.squeeze())
                accuracy += accuracy_score(output.squeeze(), masks.squeeze())
            # iou /= len(outputs)
            # f1_score /= len(outputs)
...
# Save the report
with open('results/results_unetplusplus_exp_B/model_report2.txt', 'w') as f:
    f.write(f'Model: SmallNestedUNet\n')
